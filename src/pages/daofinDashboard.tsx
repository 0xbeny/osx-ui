...
const DaofinDashboard: React.FC = () => {
  ...
  if (liveDao && liveAddressOrEns) {
    const daoType = liveDao?.plugins[0]?.id;
...
const DashboardContent: React.FC<DashboardContentProps> = ({
  ...
  const globalSettings = usePluginSettings(pluginAddress, pluginType);
  const electionPeriods = useDaofinElectionPeriods(pluginAddress);
  console.log({electionPeriods});
  if (electionPeriods?.data) {
    console.log();
  }
  if (!proposalCount) {
    return (
      <>
        <EqualDivide>
          {globalSettings?.data?.allowedAmounts && (
            <CardText
              title={'AllowedAmounts'}
              content={globalSettings.data.allowedAmounts.map(
                (amount: BigNumberish) => `${formatEther(amount)} XDC`
              )}
              type="label"
            />
          )}
          <br />
          <div>
            {globalSettings?.data?.xdcValidator && (
              <div>
                <CardWallet
                  address={globalSettings?.data?.xdcValidator}
                  wide={false}
                  name={'Validator Contract Address:'}
                  src={globalSettings?.data?.xdcValidator}
                />
              </div>
            )}
          </div>
          ...
          <div>
            {electionPeriods && (
              <div>
                {electionPeriods.data.map((period, index) => (
                  <CardText
                    title={`Election Periods #${index + 1}`}
                    content={`${new Date(
                      +period?.startDate?.toString() * 1000
                    ).toDateString()} ->${new Date(
                      +period?.endDate?.toString() * 1000
                    ).toDateString()}`}
                    type="label"
                  />
                ))}
...
export default withTransaction('DaofinDashboard', 'component')(DaofinDashboard);
